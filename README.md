...
| `signature-only` | 260 | 329 | 384 | **44.1%** | 40.4% | 82 (25%) |
| `combined (published)` | 606 | 332 | 38 | **64.6%** | 94.1% | 82 (25%) |

408 `flow-verified`, 1961 `signature-based` findings, on engine 0.5.0 (2026-09-20).

**The green label is worth 55.0 points.** `flow-verified` runs 99.1% against `signature-based` at 44.1%. The gap is the whole claim this tool makes; it is measured here rather than asserted.
...
**`npm test` — 394 checks, annotation-driven.** Fixtures carry their own
...
|---|--:|--:|--:|--:|--:|--:|--:|
| **overall** | **1210** | **606** | **332** | **38** | **234** | **64.6%** | **94.1%** |

2766 files, 0 parse errors, 23.5 seconds, on engine 0.5.0 (2026-09-20).

**Precision is the weaker side.** 332 false positives against 38 false negatives - 8.7x as many - so the cost of this engine is triage time, not missed bugs.

Missed by category: xss 27, cmdi 11. Of the 332 false positives, **82 (25%)** carry BenchmarkJava's constant-branch decoy - the `if ((7 * 42) - num > 200)` shape that needs constant folding plus branch feasibility to see through, which the limitations list says this engine does not do. The remainder are ours.
