 ...
 *
 * Defuse promises one thing that Snyk / Semgrep / SonarQube do not
...
export const ENGINE_CAPABILITIES = {
  ...
  notImplemented: [
    ...
    'Call chains deeper than four functions are not followed, and a call that resolves by name to a function already being followed on the same path - recursion, or a same-named method on another object - is not re-entered. At either stop the value is carried on as an ASSUMPTION that the unread call preserves it, and the finding names the call and why it was not read. So a sanitiser hidden past a stop is missed and the line is reported; before this was fixed the stop declared the value clean, which cost BenchmarkJava 38 real vulnerabilities.',
    'Calls are resolved by NAME, with two checks where the language makes them sound: in Java, Go, C and C++ a call must pass a number of arguments the target accepts, and in Java a call on a receiver of a declared type only reaches methods declared in that type. An interface-typed receiver therefore reaches no body at all, and the value is carried past it as a named assumption.',
    ...
    'Framework awareness is PER-FRAMEWORK and partial. Recognised: Express/Koa request objects, Flask/Django/DRF request objects, Spring and JAX-RS parameter annotations, servlet getters, net/http and Gin/Echo context values, PHP superglobals and $request accessors. NOT recognised, and each one is a silent zero: any house-built REST layer (elasticsearch\'s own RestRequest is 783 uses in one repository), Micronaut, Quarkus outside JAX-RS, Vert.x, Play, NestJS decorators, FastAPI typed parameters, and Express ROUTING itself - a value that only becomes attacker-controlled through a framework binding we do not list is never seen as a source. The report now prints how many sources were recognised, so a zero is visible rather than reading as a clean bill.',
    ...
    'MEMORY SCALES WITH THE SOURCE, at roughly 7x its bytes plus a fixed ~300MB for Node and the seven WebAssembly grammars. Syntax trees live in a WASM heap that JavaScript garbage collection cannot touch, so they are freed explicitly: the scan holds as many as DEFUSE_TREE_BUDGET_MB allows and parses a file again if something needs it after its tree was dropped. Measured on one 54MB corpus, same binary, only the budget changed: 1474MB peak holding everything, 716MB with a 256MB tree budget, findings byte-identical, 7% slower. That puts the practical ceiling near 1.1GB of source on an 8GB machine. A scan that does not fit gets SLOWER rather than being killed, and the report says so - diagnostics.treeMemory.reparses above zero means files were parsed more than once. THE PREVIOUS VERSION OF THIS NOTE WAS WRONG, and is recorded here rather than quietly deleted: it said trees were never freed "because cross-file tracing needs to resolve into any file at any time", and cited a 120MB ceiling. Cross-file had nothing to do with it - measured, --no-cross-file used 535.8MB against 528.8MB with it on, because nothing called delete() in either mode. The ceiling was real; the explanation for it was invented.',
    'Expressions nested deeper than 400 levels are not walked. Hand-written code never reaches this; generated, minified and stress-test files do. The count is reported as a blind spot rather than silently skipped - an earlier version had no limit at all and one such file killed an entire 25,809-file scan with a stack overflow, losing every finding in it.',
