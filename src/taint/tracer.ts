...
export function traceFile(
  ...
  const evaluate = (node: Node | null, scope: Scope, depth = 0): Taint | null => {
    ...
    if (node.type === 'ternary_expression' || node.type === 'conditional_expression') {
      /*
       * When the condition is decidable, only the branch that runs can carry a
       * value. `(7*18)+106 > 200 ? "constant" : param` is the constant, every
       * time, and the taint on the other side is in unreachable code.
       *
       * When it is not decidable, fall through to the original behaviour: take
       * whichever branch is dirty, because either might run.
       */
      const parts = ternaryParts(node);
      const truth = conditionTruth(parts.condition, scope.body);
      if (truth !== undefined) {
        const taken = truth ? parts.consequence : parts.alternative;
        const taint = taken ? evaluate(taken, scope, depth + 1) : null;
        return taint ? addStep(taint, node, 'returned from one branch of a conditional') : null;
      }
      for (const child of node.namedChildren) {
        if (!child) continue;
  ...
  const underCondition = (node: Node, body: Node): boolean => {
    ...
    while (current && current !== body) {
      if (CONDITIONAL_NODES.has(current.type)) {
         ...
         * dead arm and still refuse to call the live arm proven, which would be
         * a strange half-answer: certain enough to drop one side, not certain
         * enough to trust the other.
         */
        if (current.type === 'if_statement' && conditionTruth(current.childForFieldName('condition'), body) === true) {
          current = current.parent;
          continue;
        }
        return true;
  ...
  // assignmentParts and ASSIGNMENT_NODES live in fold.ts, which needs them too.
  ...
  const walk = (node: Node, scope: Scope, depth = 0): void => {
     ...
     * This decides which STATEMENTS happen at all, which is what the `if/else`
     * form of the same decoy needs - a write inside the dead arm should never
     * be recorded, rather than recorded and then reasoned about.
     *
     * Nothing is skipped unless the condition folded to a definite answer, so
     * an undecidable `if` still has both arms walked exactly as before.
     */
    if (node.type === 'if_statement') {
      ...
      if (dead) {
         ...
         * `child !== dead` was true even for the very node we meant to skip -
         * the branch folded correctly and then got walked anyway.
         */
        for (const child of node.namedChildren) {
          if (!child) continue;
          if (child.startIndex === dead.startIndex && child.endIndex === dead.endIndex) continue;
